...
const useConstants = () => {
  const [constants, setConstants] = useState(() => {
    ...
    if (typeof window !== 'undefined') {
      try {
        const saved = localStorage.getItem('KeepMyKit-constants');
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      try {
        localStorage.setItem('KeepMyKit-constants', JSON.stringify(constants));
      } catch (error) {
  ...
  const resetConstants = () => {
    ...
    if (typeof window !== 'undefined') {
      localStorage.removeItem('KeepMyKit-constants');
    }
  };
